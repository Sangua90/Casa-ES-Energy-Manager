...
class TestUIContract(unittest.TestCase):
    ...
    def test_optional_fields_are_visibly_marked(self) -> None:
        config_optional = {"phase_l1_power_sensor", "phase_l2_power_sensor", "phase_l3_power_sensor", "pv_potential_power_sensor", "pv_forecast_remaining_today_sensor", "pv_forecast_current_hour_sensor", "pv_forecast_next_hour_sensor", "pv_forecast_today_sensor", "pv_forecast_tomorrow_sensor", "weather_entity", "emergency_charge_start_script", "emergency_charge_stop_script", "extra_context_sensors", "ai_task_entity"}
        managed_optional = {"power_sensor", "expected_runtime_minutes", "min_on_minutes", "min_off_minutes", "schedule_deadline", "start_after", "end_before"}
        monitored_optional = {"emergency_entity", "resume_entity"}
        all_config_labels = {}
        for step in self.it["config"]["step"].values(): all_config_labels.update(step.get("data", {}))
        for key in config_optional: self.assertIn("*", all_config_labels[key], key)
        all_managed_labels = {}
        for step in self.it["config_subentries"]["managed_device"]["step"].values(): all_managed_labels.update(step.get("data", {}))
        for key in managed_optional: self.assertIn("*", all_managed_labels[key], key)
        all_monitored_labels = {}
        for step in self.it["config_subentries"]["monitored_load"]["step"].values(): all_monitored_labels.update(step.get("data", {}))
        for key in monitored_optional: self.assertIn("*", all_monitored_labels[key], key)
    ...
    def test_add_actions_are_unambiguous(self) -> None:
        ...
        self.assertNotEqual(managed, monitored); self.assertIn("gestito", managed); self.assertIn("controllabile", managed); self.assertIn("monitorato", monitored); self.assertIn("protezione", subentries["monitored_load"]["entry_type"].lower())
    ...
    def test_managed_flow_has_conditional_climate_step_without_ev_or_dependency(self) -> None:
        ...
        self.assertTrue({"user", "reconfigure", "climate", "constraints"} <= set(managed_steps)); self.assertNotIn("advanced", managed_steps); self.assertTrue({"user", "reconfigure"} <= set(monitored_steps))
        all_fields = set()
        for step in managed_steps.values(): all_fields.update(step.get("data", {}))
        removed = {"requires_entity", "dynamic_current", "current_entity", "min_current_a", "max_current_a", "ev_soc_sensor", "ev_connected_sensor", "ev_target_soc"}
        self.assertTrue(removed.isdisjoint(all_fields)); self.assertIn("device_type", all_fields); self.assertIn("mode_climate_entity", all_fields)
        managed_source = (INTEGRATION / "managed_device_flow_v1.py").read_text(encoding="utf-8")
        self.assertIn('step_id="user"', managed_source); self.assertIn("async_step_climate", managed_source); self.assertNotIn("async_step_advanced", managed_source)
    ...
    def test_monitored_load_keeps_one_category_with_optional_emergency_control(self) -> None:
        ...
        for step_name in ("user", "reconfigure"):
            ...
            for key in ("name", "power_sensor", "phase", "enabled", "emergency_entity", "resume_entity"): self.assertIn(key, fields)
        description = monitored["step"]["user"]["description"].lower(); self.assertIn("emergenza", description); self.assertIn("mai", description); self.assertIn("batteria", description)
        source = (INTEGRATION / "monitored_load_flow.py").read_text(encoding="utf-8"); self.assertIn("CONF_MONITORED_LOAD_EMERGENCY_ENTITY", source); self.assertIn("CONF_MONITORED_LOAD_RESUME_ENTITY", source)
    ...
    def test_climate_device_type_is_clear_and_mode_reference_is_required(self) -> None:
        ...
        self.assertIn("climate", selector_options); self.assertIn("pompa di calore", selector_options["climate"].lower()); self.assertIn("mode_climate_entity", managed["climate"]["data"]); self.assertNotIn("*", managed["climate"]["data"]["mode_climate_entity"])
    ...
    def test_nominal_power_is_explained_as_estimate_and_fallback(self) -> None:
        ...
        label = managed["data"]["nominal_power_w"].lower(); description = managed["data_description"]["nominal_power_w"].lower()
        self.assertIn("fallback", label); self.assertIn("iniziale", description); self.assertIn("apprendimento", description)

    def test_priority_and_optional_cycle_contract(self) -> None:
        managed = self.it["config_subentries"]["managed_device"]["step"]["user"]
        self.assertIn("1-100", managed["data"]["priority"]); self.assertIn("1 a 100", managed["data_description"]["priority"])
        for key in ("expected_runtime_minutes", "min_on_minutes", "min_off_minutes"):
            self.assertIn("*", managed["data"][key]); self.assertIn("Facoltativo", managed["data_description"][key])
