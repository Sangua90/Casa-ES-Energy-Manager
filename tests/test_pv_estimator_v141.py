...
def test_measured_only_fallback_is_exact():
    ...
    data = estimator.update(measured_pv_w=2200, load_w=1800, grid_power_w=0, battery_power_w=400, custom_forecast_w=None, provider_forecast_w=None, inverter_limit_w=10000)
...
def test_custom_forecast_has_stronger_initial_weight():
    estimator = AdaptivePVPotentialEstimator()
    data = estimator.update(measured_pv_w=1800, load_w=1800, grid_power_w=0, battery_power_w=0, custom_forecast_w=5200, provider_forecast_w=7000, inverter_limit_w=10000)
...
def test_estimator_never_goes_below_measured_pv():
    estimator = AdaptivePVPotentialEstimator()
    data = estimator.update(measured_pv_w=4800, load_w=5000, grid_power_w=200, battery_power_w=0, custom_forecast_w=3500, provider_forecast_w=3900, inverter_limit_w=10000)
...
def test_clear_grid_import_creates_learning_samples():
    estimator = AdaptivePVPotentialEstimator()
    for _ in range(30):
        data = estimator.update(measured_pv_w=4500, load_w=6000, grid_power_w=1500, battery_power_w=0, custom_forecast_w=4700, provider_forecast_w=6000, inverter_limit_w=10000)
...
def test_zero_export_clipping_does_not_train_forecasts_down():
    ...
    data = estimator.update(measured_pv_w=1500, load_w=1500, grid_power_w=0, battery_power_w=0, custom_forecast_w=5200, provider_forecast_w=5600, inverter_limit_w=10000)
...
def test_inverter_limit_caps_potential():
    estimator = AdaptivePVPotentialEstimator()
    data = estimator.update(measured_pv_w=5000, load_w=5000, grid_power_w=0, battery_power_w=0, custom_forecast_w=13000, provider_forecast_w=12000, inverter_limit_w=10000)
