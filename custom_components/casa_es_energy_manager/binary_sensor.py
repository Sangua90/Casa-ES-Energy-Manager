...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    async_add_entities(CasaESBinarySensor(coordinator, entry, description) for description in BINARY_SENSORS)


class CasaESBinarySensor(CoordinatorEntity[CasaESEnergyCoordinator], BinarySensorEntity):
    _attr_has_entity_name = True

    def __init__(self, coordinator: CasaESEnergyCoordinator, entry: ConfigEntry, description: CasaESBinaryDescription) -> None:
