...
def evaluate_managed_devices(
    ...
    *,
    ...
    """Evaluate AUTO devices while respecting manual modes and anti-cycling."""
    ...
    for original in devices:
        ...
        runtime = max(_number(item.get("expected_runtime_minutes"), 60.0), 1.0)
        configured_power = max(_number(item.get("nominal_power_w")), 0.0)
        admission_power = max(_number(item.get("admission_power_w"), configured_power), 0.0)
    ...
    for decision in result.get("dry_run_decisions", []):
        ...
        decision["can_auto_stop"] = not decision.get("running") or elapsed_seconds >= min_on_minutes * 60.0
        ...
        if mode == DEVICE_MODE_OVERRIDE:
            ...
            decision["reason"] = "Modalità OFF: dispositivo escluso dalla gestione e da mantenere spento."
            decision["would_start"] = False
    ...
    result["managed_devices_override"] = sum(
        1 for item in decisions if item.get("management_mode") == DEVICE_MODE_OVERRIDE
    )
